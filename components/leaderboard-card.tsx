...
export function LeaderboardCard() {
  ...
  const rest = leaderboard.slice(3).filter((entry) => entry.riotId.toLowerCase().includes(search.toLowerCase()))
    ...
    <div className="glass-card rounded-2xl p-6">
      ...
      {activeTab === "global" ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-center">
          ...
          <p className="text-xs text-muted-foreground">Connect with friends to compete!</p>
        </div>
      )}
    </div>
  )
      ...
      <div className="relative mb-2">
        <Avatar
          className={`
            ...
            ${position === 1 ? "ring-yellow-500" : position === 2 ? "ring-slate-400" : "ring-amber-700"}
          `}
          style={{ boxShadow: glows[position] }}
        >
          <AvatarImage src={entry.avatarUrl || "/placeholder.svg"} />
          <AvatarFallback className="bg-secondary">{entry.riotId.slice(0, 2).toUpperCase()}</AvatarFallback>
        </Avatar>
        {position === 1 && <Crown className="absolute -top-3 left-1/2 -translate-x-1/2 w-5 h-5 text-yellow-500" />}
      </div>
      <div
        className={`
        ...
        `}
      >
        <span className="text-xs font-medium truncate w-full text-center px-1">{entry.riotId}</span>
        <span className="text-xs text-muted-foreground">{entry.points.toLocaleString()}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div
      className={`
        ...
        ${highlighted || entry.isCurrentUser ? "bg-primary/10 border border-primary/30" : "hover:bg-secondary/30"}
      `}
    >
      <span className="w-6 text-center text-sm font-medium text-muted-foreground">{entry.rank}</span>
      <Avatar className="w-8 h-8">
        <AvatarImage src={entry.avatarUrl || "/placeholder.svg"} />
        <AvatarFallback className="bg-secondary text-xs">{entry.riotId.slice(0, 2).toUpperCase()}</AvatarFallback>
      </Avatar>
      <div className="flex-1 min-w-0">
        <p className="font-medium text-sm truncate">{entry.riotId}</p>
      </div>
      <span className="text-sm text-muted-foreground">{entry.points.toLocaleString()}</span>
      <div className="w-4">
        {entry.change === "up" && <TrendingUp className="w-4 h-4 text-green-400" />}
        {entry.change === "down" && <TrendingDown className="w-4 h-4 text-red-400" />}
